...
var fittsTest = {
	...
	downloadResult: function() {
		...
		if(this.experimentStep == 0) {
			// remove max and min
			this.mouseData = removeMinMax(this.mouseData)

			const titles = ['time','distance','width','ID'];
			csvContent = "data:text/csv;charset=utf-8," + titles.join(",") + "\r\n" + this.mouseData.map(obj => Object.values(obj).join(",")).join("\n");
			file_name = 'data_mouse.csv'
		}
		else if(this.experimentStep == 1) {
			// remove max and min
			this.gamepadData = removeMinMax(this.gamepadData)

			const titles = ['time','distance','width','ID'];
			csvContent = "data:text/csv;charset=utf-8," + titles.join(",") + "\r\n" + this.gamepadData.map(obj => Object.values(obj).join(",")).join("\n");
			file_name = 'data_gamepad.csv'
		}
		else {
			// to do: cursor area

			// remove max and min
			this.gamepadCursorData = removeMinMax(this.gamepadCursorData)

			const titles = ['time','distance','width','cursor_diameter','ID'];
			csvContent = "data:text/csv;charset=utf-8," + titles.join(",") + "\r\n" + this.gamepadCursorData.map(obj => Object.values(obj).join(",")).join("\n");
			file_name = 'data_gamepad_cursor.csv'
		}
	...
	getAreaCursor: function() {
		/*var mouse_average_time_per_id = {}
		var gamepad_average_time_per_id = {}
		var area_cursor = []

		this.IDs.forEach(function(id) {
			mouse_average_time_per_id[id] = {nb: 0, value: 0, avg: 0}
			gamepad_average_time_per_id[id] = {nb: 0, value: 0, avg: 0}
		});

		this.mouseData.forEach(function(obj){
			mouse_average_time_per_id[obj.id].value += obj.time
			mouse_average_time_per_id[obj.id].nb++
		});

		this.gamepadData.forEach(function(obj){
			gamepad_average_time_per_id[obj.id].value += obj.time
			gamepad_average_time_per_id[obj.id].nb++
		});

		this.IDs.forEach(function(id) {
			mouse_average_time_per_id[id] = mouse_average_time_per_id[id].value / mouse_average_time_per_id[id].nb
			gamepad_average_time_per_id[id] = gamepad_average_time_per_id[id].value / gamepad_average_time_per_id[id].nb

			// area cursor pour compenser
		});
		// pour chaque ID
		// - temps moyen mouse / gamepad
		// - area cursor pour composer

		// moyenne des area cursor*/
		...
		if (distance({x: x, y: y}, this.target) - (this.isoParams.cursorDiameter / 2) < (this.target.w / 2)) {
			...
			if(!this.training) {
				...
				if (dt < MAX_TIME)  // skip if obvious outlier
				{
					...
					/*if(this.gamepadMode) {
						this.gamepadData.push({time: dt, distance: dist, width: data.target.w, cursor_diameter: this.isoParams.cursorDiameter})
					}
					else {
						this.mouseData.push({time: dt, distance: dist, width: data.target.w})
					}*/
				}
			}
			...
			if (this.isoParams.randomize && this.currentCount >= this.isoPositions.length) {
				...
				if(!this.training && fittsTest.IDsArrayIndex == 0) {
					if(this.experimentStep == 0) {
						...
						// todo: passer en console
						$('#experiment_step_title').text('Take your gamepad!')
						connectGamepad()
					}
					else if(this.experimentStep == 1) {
						console.log(this.gamepadData)
						// todo: calcul du area cursor pour compenser
						this.isoParams.cursorDiameter = this.getAreaCursor()
					}
					else {
						console.log(this.gamepadCursorData)
						// todo: repasser en souris
						$('#experiment_step_title').text('Take your mouse!')
						disconnectGamepad()
				...
				// to do : decrease cursorDiameter in gamepadData
				// to do : enlever le random parameter et voir comment on calcule width et dist pour avoir le bon ID (environ)
			}
			else {
				this.currentCount++;
				this.generateTarget();			
...
{
	if( 
		(!fittsTest.training && fittsTest.experimentStep == 0)
		|| (fittsTest.training && !fittsTest.gamepadMode)
	) {
...
function mouseMovedGamepad() {
	...
	if( 
		connected_gamepad != null && (
		(!fittsTest.training && (fittsTest.experimentStep == 1 || fittsTest.experimentStep == 2))
		|| (fittsTest.training && fittsTest.gamepadMode))
	) {
		...
		fittsTest.mouseMoved(left - offset.left, top - offset.top)
	}
}
...
function connectGamepad() {
	...
	if(connected_gamepad != null) {
		connected_gamepad.on('button0', mouseClickedGamepad);
	}
}
...
$('#training_mode_radio').click(function() {
	fittsTest.training = true
	fittsTest.mouseData = []
	fittsTest.gamepadData = []
	fittsTest.gamepadCursorData = []
	$('#experiment_trial_title').hide()
	$('#experiment_step_title').hide()
	$('#device_modes').show()
...
$('#experiment_mode_radio').click(function() {
	fittsTest.training = false
	fittsTest.mouseData = []
	fittsTest.gamepadData = []
	fittsTest.gamepadCursorData = []
	$('#device_modes').hide()
	disconnectGamepad()
	fittsTest.reinitExperiment()
